...
use \Exception;
  ...
  * Uploader Class
  *
  * Main class for uploading files
  *
  */
...
class Image {
    ...
    public function upload(){
        try {
            if(self::$file->getError() !== UPLOAD_ERR_OK) return false;
            $extension = pathinfo(self::$file->getClientFilename(), PATHINFO_EXTENSION);
            if(!in_array($extension, self::$extensions) || (self::$file->getSize() > self::$filesize) || !in_array(self::$file->getClientMediaType(), self::$mimes)) return false;
