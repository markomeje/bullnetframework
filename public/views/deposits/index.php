	    ...
	    <div class="container">
            <div class="d-flex flex-wrap justify-content-between align-items-center mb-1">
                <div class="d-flex align-items-center flex-wrap">
                    ...
                    <a href="<?= DOMAIN; ?>/deposits" class="text-muted mb-3 mr-2 bg-white shadow-sm btn btn-sm rounded-pill px-4">
                        All Deposits
                    </a>
                    <?php if(Authenticated::user()->role === 'cashier'): ?>
                        <a href="javascript:;" data-toggle="modal" data-target="#add-deposit" class="text-muted mb-3 mr-2 bg-white shadow-sm btn btn-sm rounded-pill px-4">
                            Add Deposit
                        </a>
                        <?php require VIEWS_PATH . DS . 'deposits' . DS . 'partials' . DS . 'add.php'; ?>
                    <?php endif; ?>
            ...
            <?php if(Authenticated::user()->role === 'cashier'): ?>
                <?php if(empty($allCashierDeposits)): ?>
                    <div class="alert alert-info">No deposits yet</div>
                <?php else: ?>
                    <div class="row">
                        <?php foreach($allCashierDeposits as $deposit): ?>
                            <?php require VIEWS_PATH . DS . 'deposits' . DS . 'partials' . DS . 'listings.php'; ?>
                        <?php endforeach; ?>
                    </div>
                    <?php foreach($allCashierDeposits as $deposit): ?>
                        <?php require VIEWS_PATH . DS . 'deposits' . DS . 'partials' . DS . 'edit.php'; ?>
                    <?php endforeach; ?>
                <?php endif; ?>
