(function ($) {
    ...
    $('.process-password-reset-form').submit(function(event){
        event.preventDefault();
        var form = $(this);
        var button = $('.process-password-reset-button');
        var spinner = $('.process-password-reset-spinner');
        var message = $('.process-password-reset-message');
        ...
        request.done(function(response){
            if (response.status === 0 && response.field === 'processemail') {
                handleButton(button, spinner);
                handleErrors($('.processemail'), $('.processemail-error'), response.info);
    ...
    $('.password-reset-form').submit(function(event){
        ...
        request.done(function(response){
            if (response.status === 0 && response.field === 'token') {
                handleButton(button, spinner);
                handleErrors($('.token'), $('.token-error'), response.info);
